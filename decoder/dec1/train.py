...
import json
import tqdm
from decoder.dec1.config import DatasetConfig, TrainConfig, ModelConfig            
from decoder.dec1.inference import augment_colors_batch, augmented_inference_batched, augmented_inference_batched_with_voting
...
def _get_model(mcfg : EncoderConfig, tcfg : TrainConfig) -> ARCPureDecoder :
    model = ARCPureDecoder(mcfg).to(tcfg.device)
    return model


def _get_optimizer(model : ARCPureDecoder, tcfg : TrainConfig) :
